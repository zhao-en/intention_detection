...
def train_lstm(model, x_train, y_train, x_test, y_test):
    ...
    f = open('D:/PyCharm 2019.2/projects/lstm_softmax/output/bert_gru_softmax_history', 'w')
    ...
    with open('output/bert_gru_softmax.yml', 'w') as outfile:
        outfile.write(yaml.dump(yaml_string, default_flow_style=True))
    model.save('output/bert_gru_softmax.h5')
...
def test():
    ...
    # 五个意图统计
    result = []
    lstm_model = load_model('D:\\PyCharm 2019.2\\projects\\lstm_softmax\\output\\bert_gru_softmax.h5')
    for path in paths:
