...
def create_dictionaries(model=None,
    ...
    if (combined is not None) and (model is not None):
        ...
        fopen = open('D:/PyCharm 2019.2/projects/lstm_softmax/output/bert_lstm_softmax_vocab', 'w', encoding='utf-8')
        fopen.write(str(w2indx))
        fopen.close()
...
# debug
        ...
        # pad 补上0
        combined = sequence.pad_sequences(combined,maxlen=18)
        global input_length
...
def train_lstm(model, x_train, y_train, x_test, y_test):
    ...
    # f = open('D:/PyCharm 2019.2/projects/lstm_softmax/output/bert_lstm_softmax_history', 'w')
    ...
    with open('output/bert_lstm_softmax.yml', 'w') as outfile:
        outfile.write(yaml.dump(yaml_string, default_flow_style=True))
    model.save('output/bert_lstm_softmax.h5')
...
def input_transform(testdata):
    ...
    combined2 = sequence.pad_sequences(list(combined2),maxlen=18)
    return combined2
