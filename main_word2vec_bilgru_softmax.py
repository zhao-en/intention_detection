...
# 1170/1170 [==============================] - 6s 5ms/step - loss: 0.1082 - acc: 0.9581 - val_loss: 1.1591 - val_acc: 0.7611
...
def build_model(n_symbols, embedding_weights):
    ...
    model.add(Bidirectional(GRU(output_dim=50, activation='relu',inner_activation='hard_sigmoid')))
...
def train_lstm(model, x_train, y_train, x_test, y_test):
    ...
    # f = open('D:/PyCharm 2019.2/projects/lstm_softmax/output/word2vec_bigru_softmax_history', 'w')
    ...
    with open('output/word2vec_bigru_softmax.yml', 'w') as outfile:
        outfile.write(yaml.dump(yaml_string, default_flow_style=True))
    model.save('output/word2vec_bigru_softmax.h5')
...
def test():
    ...
    # 五个意图统计
    result = []
    lstm_model = load_model('D:\\PyCharm 2019.2\\projects\\lstm_softmax\\output\\word2vec_bigru_softmax.h5')
    for path in paths:
